...
// Use this for type-safe require of yaml if needed
let YAML: any;
try {
  YAML = require("yaml");
} catch {
  // handled in parseCouncilConfig
}
...
interface CouncilSettings {
  exclude_chairman_from_members?: boolean;
  timeout?: number;
}

interface CouncilConfig {
  council: {
    chairman: { role: string };
...
function detectHostRole(): string {
...
function resolveAutoRole(role: string, hostRole: string): string {
...
function parseCouncilConfig(configPath: string): CouncilConfig {
...
function computeTerminalDoneCount(counts: any): number {
  const c = counts || {};
  return (
    Number(c.done || 0) +
...
function buildCouncilUiPayload(statusPayload: StatusPayload): any {
...
function computeStatusPayload(jobDir: string): StatusPayload {
  ...
  for (const m of members) {
    const state = String(m.state || "unknown") as keyof typeof totals;
...
function cmdStart(options: any, prompt: string): void {
  const configPath =
  ...
  for (const member of members) {
    ...
    // Use npx ts-node for the TS worker
    const binary = "npx";
    const workerArgs: string[] = ["ts-node", "-T", WORKER_PATH];
...
function cmdStatus(options: any, jobDir: string): void {
...
function parseWaitCursor(value: string): any | null {
...
function asWaitPayload(statusPayload: StatusPayload): any {
  const members = Array.isArray(statusPayload.members)
    ? statusPayload.members
    : [];
  return {
...
function cmdWait(options: any, jobDir: string): void {
  const resolvedJobDir = path.resolve(jobDir);
...
function cmdResults(options: any, jobDir: string): void {
  const resolvedJobDir = path.resolve(jobDir);
  const jobMeta = readJsonIfExists(path.join(resolvedJobDir, "job.json"));
  const membersRoot = path.join(resolvedJobDir, "members");
...
function cmdStop(_options: any, jobDir: string): void {
  const resolvedJobDir = path.resolve(jobDir);
  const membersRoot = path.join(resolvedJobDir, "members");
  if (!fs.existsSync(membersRoot))
...
function cmdClean(_options: any, jobDir: string): void {
  const resolvedJobDir = path.resolve(jobDir);
