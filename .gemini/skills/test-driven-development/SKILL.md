...
2. **Contract Verification**: Verify the outgoing request (URL, Method, Body, Headers) matches the expected contract.
3. **Domain Organization**: Store mock handlers in `test/mocks/[domain]/`.
4. **No implementation-leaking mocks**: Tests should not know *how* the service works internally, only what it sends over the wire.

<Good>
```typescript
// test/mocks/payment/handlers.ts
export const handlers = [
  http.post('https://api.stripe.com/v1/charges', async ({ request }) => {
    const body = await request.json();
    if (body.amount < 0) return new HttpResponse(null, { status: 400 });
    return HttpResponse.json({ id: 'ch_123', status: 'succeeded' });
  }),
];

// In your test
test('sends correct payload to Stripe', async () => {
  await stripeProvider.charge({ amount: 1000, currency: 'usd' });
  // Verification happens at the network level via MSW interceptions or spy on handlers
});
```
</Good>
...
**If you cannot make the test pass (GREEN) after 2-3 attempts:**
1.  **Stop writing code.**
2.  **Activate `systematic-debugging` skill.**
3.  Use the debugger to understand *why* the implementation is failing.
4.  Once understood, return to TDD to fix it.

**If the test passes but breaks other tests:**
1.  **Revert** the last change.
2.  **Activate `systematic-debugging`** to investigate the regression.

## Testing Anti-Patterns

When adding mocks or test utilities, read @assets/testing-anti-patterns.md to avoid common pitfalls:
