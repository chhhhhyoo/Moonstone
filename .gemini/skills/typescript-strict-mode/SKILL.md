...
### Explicit Entity Typing (TypeORM)

To ensure compatibility between MySQL (Production) and SQLite (Tests), **ALWAYS** define property types explicitly in TypeORM entities. Do not rely on implicit column types or `Column()` without parameters.

**❌ WRONG (Implicit/Unsafe):**
```typescript
@Column()
isActive: boolean; // Might fail in SQLite if defaults differ
```

**✅ CORRECT (Explicit/Safe):**
```typescript
@Column({ type: 'boolean', default: false })
isActive!: boolean; // Non-null assertion (!) used as TypeORM initializes it
```

Refer to `notes/09-Explicit-Entity-Typing.md` for the full rationale and examples.
